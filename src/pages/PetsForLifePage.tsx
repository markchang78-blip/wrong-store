...
const featureSection = {
  title: 'PETS ROCK WORLD',
  description: 'PETS ROCK WORLD has been rebuilt from the ground up to streamline the process of discovering, collecting, and showcasing our unique brand of street-inspired pet culture. We\'ve updated the experience to give our community more flexibility in how they represent both their love for pets and the bold, creative energy that defines our brand.',
  image: '/handbook-hero.png',
};

// ====== Two additional Image+Text blocks (below feature section) ======
const contentBlocks = [
  {
    id: 'block-1',
    title: 'HEALTH & WELLNESS',
    description: 'Your pet deserves the best care. From nutrition tips to daily exercise routines, we provide expert guidance to keep your furry friends happy, healthy, and full of life. A well-cared-for pet is a happy pet.',
    image: '/petsforlife-care.jpg',
    reverse: true,
  },
  {
    id: 'block-2',
    title: 'COMMUNITY FIRST',
    description: 'Join a global community of pet lovers who share your passion. From local meetups to online events, Pets Rock brings people together through their love for animals. Connect, share, and grow with us.',
    image: '/petsforlife-community.jpg',
    reverse: false,
  },
];

export default function PetsForLifePage() {
  ...
  return (
    <div className="min-h-screen bg-[#F4F1E8]">
      ...
      <section className="relative w-full h-[360px] md:h-[540px] lg:h-[660px] xl:h-[720px] 2xl:h-[780px] overflow-hidden">
        ...
        {/* Arrows — positioned at image center */}
        <button
          onClick={goPrev}
      ...
      {/* ====== 5. TWO IMAGE+TEXT BLOCKS (like The Unlikely Entertainment) ====== */}
      ...
      {/* ====== 6. MARQUEE BANNER — full opacity background, taller ====== */}
      <section className="relative w-full h-[600px] md:h-[700px] lg:h-[800px] overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center bg-fixed" style={{ backgroundImage: 'url(/petsforlife-bg.jpg)' }} />
        <div className="absolute inset-0 flex items-center overflow-hidden pointer-events-none">
          <div className="animate-marquee-slow whitespace-nowrap flex">
            {[...Array(4)].map((_, i) => (<span key={i} className="text-[60px] md:text-[80px] font-extrabold text-white/30 uppercase tracking-tight mx-8">PETS FOR LIFE</span>))}
          </div>
        </div>
        <div className="absolute inset-0 flex items-center justify-center z-10">
          <div className="bg-white/85 backdrop-blur-sm w-[90%] max-w-[500px] p-8 md:p-12 text-center">
            <p className="text-sm md:text-base text-[#333] leading-relaxed">A lifetime of love, laughter, and furry companions. Every moment with your pet is a story worth cherishing.</p>
            <a href="#/news" className="inline-flex items-center gap-2 mt-6 text-xs uppercase tracking-[0.05em] font-medium text-black group relative">Explore Pets For Life <ArrowRight size={14} /></a>
          </div>
        </div>
      </section>

      {/* ====== 7. CTA SECTION — solid white banner ====== */}
      <section className="relative w-full h-[400px] md:h-[500px] bg-white overflow-hidden">
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-6">
          <h2 className="font-extrabold text-black uppercase mb-4" style={{ fontSize: 'clamp(28px, 5vw, 56px)', letterSpacing: '-0.02em' }}>A LIFETIME OF LOVE</h2>
          <p className="text-sm md:text-base text-brand-text-secondary max-w-[480px] mb-8">Celebrate the bond that lasts forever. Every moment with your pet is a story worth cherishing.</p>
          <a href="#/news" className="inline-flex items-center px-8 py-4 bg-black text-white text-sm uppercase tracking-wider font-medium hover:bg-white hover:text-black border border-black transition-colors">Explore Pets For Life</a>
        </div>
      </section>
